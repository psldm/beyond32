...
def tex_decomposition(dec: Dict[str, int], order: List[str] | None = None) -> str:
    ...
    for k in keys:
        ...
        if not m:
            continue
        parts.append((f"{m}\\," if m > 1 else "") + tex_irrep(k))
...
def tex_poly(s: str) -> str:
    """A polynomial string from sympy ('x**3 - 3*varphi*x*z**2 + ...') -> LaTeX."""
    s = str(s).replace("varphi", r"\varphi")
    s = re.sub(r"\*\*(\d+)", r"^{\1}", s)
    s = s.replace("*", " ")
    s = re.sub(r"(\d+)/(\d+)", r"\\tfrac{\1}{\2}", s)
    s = s.replace("sqrt(5)", r"\sqrt5")
    return s
...
def eq_bases(res) -> str:
    bf = res["harmonics"]["basis_functions"]
    ...
    for key, (l, lab) in _BASIS_LABELS.items():
        funcs = bf[key]
        items = [f"{_tex_fname(n)} &= {tex_poly(p)}" for n, p in funcs.items()]
        lines.append(f"\\ell={l},\\ {lab}:\\quad " + r" \\ ".join(items) + r" \\")
    lines.append(f"P_6 &= {tex_poly(res['harmonics']['P6'])} \\\\")
    hx = res["harmonics"]["hexad"]
    lines.append(rf"\sum_i (\hat a_i\cdot\hat n)^6 &= {tex_num(hx['c'])}\,P_6 + {tex_num(hx['e'])}\,r^6 \quad(\text{{Eq. 10 on the unit sphere}})")
...
def eq_invariants(res) -> str:
    ...
    def comb(d):
        ...
        for k, v in d.items():
            if v in ("0", 0):
                continue
            parts.append(f"{tex_num(v)}\\,{_tex_inv(k)}")
