...
**What changed in v1.1.0 (2026-09-22).** Appendix B of the paper was rewritten: the point
group of the Ta1.6Te dodecagonal quasicrystal is D6d (−12m2; Yamamoto, Acta Cryst. A 60, 142
(2004)), not D12.  The module `beyond32/d12.py` was replaced by `beyond32/d6d.py`, which
constructs D6d explicitly as exact 3×3 matrices in Yamamoto's orientation (generated by
S12 = diag(1, 1, −1)·R_z(30°) and the two-fold rotation about the in-plane axis at 15°; order
24, no inversion, no horizontal mirror, no true twelve-fold rotation, nine classes) and
reproduces Table 10: the character table, the singlet and triplet pair functions on the
in-plane circle with the basis functions of every irrep, the antisymmetric spin-orbit vector
(sin 5φ, cos 5φ, 0), the restriction D6d ↓ D2d and the 30° orbit of an in-plane field
direction.  `tests/test_d6d.py` (19 tests) and `tables/tab_D6d.tex` replace `tests/test_d12.py`
and `tables/tab_D12.tex`, and `beyond32 d6d` prints the Appendix B summary.  The D6d material
has no counterpart among the original scripts (their `d12.py` computed the D12 material of the
former Section 10), so its reference values are those of the rewritten Appendix B; every other
section of `results.json` and every other LaTeX fragment is unchanged from v1.0.0.
...
following are pinned literally in `tests/` (115 tests); `beyond32 check` recomputes a
selection of about 40 key items and compares them with the paper.
...
| D6d (−12m2): the 24 exact matrices generated by S12 and C2′(15°); no inversion, no σ_h, no true C12; 9 classes; character table (B1 = det g, B2 = g_zz); singlet m = 0, 2, 4, 6 → A1, E2, E4, B1 + B2 and triplet m = 1, 3, 5 → B1 + B2 + E4, E2 + E4, A1 + A2 + E2 (in-plane), E1, E3, E5 (d_z) with every basis function; g ∝ (sin 5φ, cos 5φ, 0); D6d ↓ D2d; 30° field orbit | Appendix B, Table 10 | `test_d6d` |
...
| `d12.py` (D12 material of the former Section 10) | 1 s | `d6d` (v1.1.0: the D6d material of Appendix B; no legacy counterpart) | < 1 s |
...
sympy 1.14); `pytest` (115 tests, including the six slow end-to-end cross-checks): 60 s, and
...
decompositions (`shells`); the D12 assignment and node conditions (`d12`, up to v1.0.0;
replaced in v1.1.0 by the D6d material of Appendix B, `d6d`); the G ground
...
* **Extras beyond the original scripts** (all checked): the Molien closed form derived from the
  ...
  shells, the second cyclic chirality in Table 8, the D6d pair functions of Appendix B
  (`d6d`, v1.1.0), and the closed form √λ = −7/20 + 47√5/30 obtained by denesting
  (`sqrtdenest`) and verified exactly.
...
* **Naming.** The dodecagonal material is Appendix B of the paper (Table 10), computed for
  the point group D6d since v1.1.0; up to v1.0.0 it was Section 10 (cross-referenced there
  as "Appendix 10") and computed for D12, which is what the original scripts' `d12.py` did.
